...
def cgpa(regd):
    global phym, chemm, mathm
    cgpa_ = 0
    sum = phym + chemm + mathm
    cgpa_ = (sum / 300) * 10
...
def sub_entry(window, regd):
    global flag1, flag2, flag3, phym, chemm, mathm
    flag = False
    if (flag1 == True and flag2 == True and flag3 == True):

        try:
            con = mysql.connect(host='localhost', user='root', password='', database='output')
            cursor = con.cursor()
            cursor.execute(
                f"UPDATE marks SET physics = {phym},chemistry={chemm},math = {mathm} WHERE regd = '{regd}'")
...
def phy():
    global phym, flag1
    phym = simpledialog.askinteger('input', 'please enter physics marks')
    flag1 = True
    # print(phym)
...
def chem():
    global chemm, flag2
    chemm = simpledialog.askinteger('input', 'please enter chemistry marks')
    flag2 = True
    # print(chemm)
...
def math():
    global mathm, flag3
    mathm = simpledialog.askinteger('input', 'please enter maths marks')
    flag3 = True
    # print(mathm)
...
def signin(window):
    ...
    login = Button(s_win, text="Login", font=('italic', 10, 'bold'), bg='green',
                   command=lambda: login_(s_win, e_username1, e_password1))
